...
public class Subsystem1 extends Subsystem {
    ...
    public void periodic() {
        ...
        // if (interf.controller.getRawButton(0))
        // {
        //     servo1.setAngle(0);
        // }
        // if (interf.controller.getRawButton(2))
        // {
        //     servo1.setAngle(2);
        // }
        ...
        // double turboSpeed = .9;
        if (interf.controller.getRawButton(8)){turboFactor=1;}
        ...
        if (Math.abs(rightSpeed) > deadZone){
            if(rightSpeed < 0){
                mainRight.set(Math.pow(rightSpeed, exponentFactor)/turboFactor);
                reserveRight.set(Math.pow(rightSpeed, exponentFactor)/turboFactor);
            }
            else{
                mainRight.set(Math.pow(rightSpeed, exponentFactor)/turboFactor);
                reserveRight.set(Math.pow(rightSpeed, exponentFactor/turboFactor));
        ...
        if (Math.abs(leftSpeed) > deadZone){
            if(leftSpeed < 0){
                mainLeft.set(Math.pow(leftSpeed, exponentFactor)/turboFactor);
                reserveLeft.set(Math.pow(leftSpeed, exponentFactor)/turboFactor);
            }
            else{
                mainLeft.set(Math.pow(leftSpeed, exponentFactor)/turboFactor);
                reserveLeft.set(Math.pow(leftSpeed, exponentFactor)/turboFactor);
